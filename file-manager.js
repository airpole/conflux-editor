...
export function renderFMList() {
  ...
  el.innerHTML = files.map(f => {
    ...
    return `<div class="fm-item" ${isCurrent ? 'style="border-color:var(--acc)"' : ''} data-action="fmLoad" data-arg="${escAttr(f.name)}">
      ...
      <button class="fm-del" data-action="fmDelete" data-arg="${escAttr(f.name)}">✕</button>
    </div>`;
  }).join('');
}
